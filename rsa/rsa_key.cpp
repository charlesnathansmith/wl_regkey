...
// From ascii key
rsa_private_key::rsa_private_key(const char* ascii_key)
{
	mp_init_multi(&_n, &_d, NULL);
	import_from_base36der(ascii_key);
}

// Copy
rsa_private_key::rsa_private_key(const rsa_private_key& rhs)
{
	mp_init_copy(&_n, &rhs._n);
	mp_init_copy(&_d, &rhs._d);
}

// Assignment
rsa_private_key& rsa_private_key::operator=(const rsa_private_key& rhs)
{
	mp_clear_multi(&_n, &_d, NULL);  // Put in known state
	mp_init_copy(&_n, &rhs._n);
	mp_init_copy(&_d, &rhs._d);
...
bool rsa_private_key::import_from_der(uint8_t* der, size_t size)
{
	...
	if (!(size = read_der_int(&_d, der, end)))		// Read private exponent d
		return false;
...
bool rsa_private_key::import_from_base36der(const char* in)
{
