...
void lic_build_tea(lic_tea& tea, reg_info& reg)
{
	...
	// Can use the LF_ flags for license restrictions
	// (eg. LF_HWID enforces hwid locking)
	...
	tea.magic = 0xe2b27878;		// Hardcoded magic numbe
}

// Encrypt lower layers and finish license_head
void lic_build_head(license_head& head, main_hash_key& m, reg_info& reg)
{
	// TEA encrypt lower layer
	tea_encrypt((uint32_t*) &head.tea, sizeof(lic_tea), m.tea_key);

	// Password encrypt lower layer
	pw_encrypt((uint32_t*) &head.tea, sizeof(lic_tea), m.password.c_str());
